...
function AddUpdatePrefectureForm({ title, formType, buttonLabel }: Props) {
  ...
  const onSubmit = async (data: any, e: any) => {
    try {
      ...
      if (formType === 'add') {
        const res = await Api.post('/prefectures/create', newData);
        if (res.status === 201) {
          AddNotification('Dodano', 'Nowa prefektura została dodana pomyślnie', 'success');
          e.target.reset();
        }
        ...
        if (defaultValues) {
          ...
          const res = await Api.patch(`/prefectures/update/${key}`, newData);
          if (res.status === 200) {
            AddNotification('Dodano', 'Prefektura została zaktualizowana pomyślnie', 'success');
          }
        }
      }
    } catch (err) {
      if (err.response.status === 409) {
        AddNotification('Błąd', 'Taka prefektura już istnieje', 'danger');
      } else {
...
export default AddUpdatePrefectureForm;
